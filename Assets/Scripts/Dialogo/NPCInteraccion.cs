...
public class NPCInteraccion : MonoBehaviour
{
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        if (other.CompareTag("Player"))
        {
            DialogoManager.Instance.dialogosSecuencia = new Queue<string>(); ;
            DialogoManager.Instance.NPCDisponible = null;
            npcButtonInteractuar.SetActive(false);
           
            UIManager.Instance.AbrirCerrarPanelInspectorQuestConEstado(false);
            
            
            DialogoManager.Instance.AbrirCerraPanelDialogo(false);
        }
    }
}
